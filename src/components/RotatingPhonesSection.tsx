...
function PhoneModel({
  ...
  const roundedBoxGeometry = useMemo(() => createRoundedBoxGeometry(8.5, 17, 1.0, 0.4, 6), []);
  const screenGeometry = useMemo(() => createRoundedBoxGeometry(8, 16, 0.1, 0.3, 6), []);
  ...
  return (
    <Float speed={1} rotationIntensity={0.05} floatIntensity={0.2}>
      <group ref={meshRef} position={position} rotation={rotation}>
        ...
        {/* Status Bar - Brighter */}
        <mesh position={[0, 7.2, 0.53]}>
          <planeGeometry args={[7.4, 0.7]} />
          <meshPhysicalMaterial 
            color={color} 
            transparent 
            opacity={0.8} 
        ...
        {appContent.map((line, index) => (
          <Text 
            key={index}
            position={[-3.5, 6.5 - index * 0.6, 0.54]} 
            fontSize={0.35} 
            color={
        ...
        <mesh position={[-3.5 + (appContent[appContent.length - 1]?.length * 0.18 || 0), 6.5 - (appContent.length - 1) * 0.6, 0.54]}>
...
function Scene() {
  return (
    <>
      ...
      <pointLight position={[40, 40, 40]} intensity={8} castShadow color="#00f5ff" />
      <pointLight position={[-40, -40, -40]} intensity={7} color="#8b5cf6" />
      <pointLight position={[40, -40, 40]} intensity={6} color="#10b981" />
      <pointLight position={[0, 0, 60]} intensity={8} color="#ffffff" />
      <spotLight 
        position={[0, 60, 0]} 
        angle={0.8} 
        penumbra={1} 
      ...
      <CircularOrbit radius={18} count={5} />
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          {[
            ...
            <motion.div
              key={index}
              className={`p-6 rounded-xl bg-gradient-to-br ${service.color} backdrop-blur-sm border ${service.accent} hover:scale-105 transition-all duration-300`}
