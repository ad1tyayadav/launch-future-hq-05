...
function PhoneModel({
  ...
  return <Float speed={1} rotationIntensity={0.05} floatIntensity={0.2}>
      <group ref={meshRef} position={position} rotation={rotation}>
        {/* Phone Frame - Premium metallic look */}
        <mesh castShadow receiveShadow geometry={roundedBoxGeometry}>
          <meshPhysicalMaterial color="#1a1a1a" metalness={0.95} roughness={0.05} clearcoat={1} clearcoatRoughness={0.02} envMapIntensity={2} />
        </mesh>
        
        {/* Screen Base - Dark but not black */}
        <mesh position={[0, 0, 0.61]} castShadow geometry={screenGeometry}>
          <meshPhysicalMaterial color="#000000" metalness={0.1} roughness={0.05} emissive="#111111" emissiveIntensity={0.5} />
        </mesh>

        {/* App Background - Much brighter for visibility */}
        <mesh position={[0, 0, 0.62]}>
          <planeGeometry args={[9, 18.4]} />
          <meshPhysicalMaterial color="#1a1a2e" transparent opacity={0.98} emissive="#16213e" emissiveIntensity={0.3} />
        </mesh>

        {/* Status Bar - Brighter */}
        <mesh position={[0, 8.5, 0.63]}>
          <planeGeometry args={[8.8, 0.8]} />
          <meshPhysicalMaterial color={color} transparent opacity={0.6} emissive={color} emissiveIntensity={0.4} />
        </mesh>

        {/* Status Bar Text - Larger and brighter */}
        <Text position={[-3.8, 8.5, 0.64]} fontSize={0.35} color="#ffffff" anchorX="left" anchorY="middle" fontWeight="bold">
          {title.split(' ')[0]} IDE
        </Text>

        <Text position={[3.8, 8.5, 0.64]} fontSize={0.35} color="#00ff00" anchorX="right" anchorY="middle" fontWeight="bold">
          ●
        </Text>

        {/* Code Content - Larger text and better contrast */}
        {appContent.map((line, index) => <Text key={index} position={[-4.2, 7.5 - index * 0.5, 0.64]} fontSize={0.28} color={line.startsWith('//') ? '#9ca3af' : line.includes('=') || line.includes(':') ? '#22d3ee' : line.includes('function') || line.includes('const') || line.includes('def') ? '#a855f7' : line.includes('accuracy') || line.includes('loss') ? '#fbbf24' : '#f3f4f6'} anchorX="left" anchorY="middle" maxWidth={8.5} lineHeight={1.1}>
            {line}
          </Text>)}

        {/* Terminal Cursor - Brighter and bigger */}
        <mesh position={[-4.2 + (appContent[appContent.length - 1]?.length * 0.15 || 0), 7.5 - (appContent.length - 1) * 0.5, 0.64]}>
          <planeGeometry args={[0.2, 0.4]} />
          <meshPhysicalMaterial color={color} emissive={color} emissiveIntensity={1.5} transparent opacity={0.9} />
        </mesh>

        {/* Side Accent Light - Brighter */}
        <mesh position={[-4.3, 0, 0.63]}>
          <planeGeometry args={[0.2, 16]} />
          <meshPhysicalMaterial color={color} emissive={color} emissiveIntensity={0.8} transparent opacity={0.8} />
        </mesh>

        {/* Bottom Terminal Bar - Brighter */}
        <mesh position={[0, -8.2, 0.63]}>
          <planeGeometry args={[8.8, 1]} />
          <meshPhysicalMaterial color="#2d3748" transparent opacity={0.95} emissive="#374151" emissiveIntensity={0.2} />
        </mesh>

        {/* Terminal Prompt - Larger and brighter */}
        <Text position={[-4.2, -8.2, 0.64]} fontSize={0.3} color={color} anchorX="left" anchorY="middle" fontWeight="bold">
...
function CircularOrbit({
  ...
  const phones = useMemo(() => {
    return services.map((service, index) => {
      ...
      return <PhoneModel key={index} position={[x, y, z]} rotation={[0, -angle + Math.PI / 2, 0]} title={service.title} subtitle={service.subtitle} color={service.color} content={service.content} />;
    });
...
function Scene() {
  return <>
      ...
      <spotLight position={[0, 60, 0]} angle={0.8} penumbra={1} intensity={5} castShadow color="#ffffff" shadow-mapSize-width={2048} shadow-mapSize-height={2048} />
  ...
  return <section className="py-32 relative overflow-hidden min-h-screen">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        duration: 0.8
          ...
          <motion.h2 className="text-5xl md:text-7xl font-bold text-white mb-4 tracking-tight" initial={{
          opacity: 0
        }} whileInView={{
          opacity: 1
        }} transition={{
          ...
          <motion.div className="w-24 h-1 bg-gradient-to-r from-cyan-400 to-purple-400 mx-auto mb-6 rounded-full" initial={{
          scaleX: 0
        }} whileInView={{
          scaleX: 1
        }} transition={{
        ...
        <div className="relative w-full h-[900px] mb-12">
          <Suspense fallback={<LoadingFallback />}>
            ...
            powerPreference: "high-performance"
              ...
              <Scene />
              <OrbitControls enablePan={false} enableZoom={false} enableRotate={true} autoRotate={false} maxPolarAngle={Math.PI / 2.2} minPolarAngle={Math.PI / 3.5} rotateSpeed={0.5} />
            </Canvas>
          </Suspense>
        </div>
